...
final class ModuleCompilerDebugController extends Controller
{
    public function __invoke(Request $request, ModuleCompiler $compiler)
    {
        $corpusRoot = storage_path('app/private/device-scan/corpus/modules');
        ...
        if ($selected !== null && ! array_key_exists($selected, $files)) {
            $error = 'Selected file was not found in the module corpus.';
            $selected = null;
        }
        ...
        return view('device-scan.debug.module-compiler', [
    ...
    private function pdfFiles(string $root): array
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (mb_strtolower($file->getExtension()) !== 'pdf') {
                continue;
            }
