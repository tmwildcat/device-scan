...
#[Signature('device-scan:promote-tenant-record {compiled_device_record_id}')]
#[Description('Copy a tenant compiled record and datasheet into central curated review records.')]
class DeviceScanPromoteTenantRecordCommand extends Command
{
    public function handle(DeviceScanPromotionService $promotion): int
    {
        ...
        if ($record->source_type !== 'tenant_private') {
            $this->error('Record is not a tenant_private compiled device record.');
